...
- 프로필별 추가 인자는 한 줄에 옵션 하나로 적고, 옵션과 값을 한 줄에 함께 써도 argv로 나눠 전달 (공백이 든 값은 `"큰따옴표"`)
...
- 게임별 실행 프로필에서 Windows 관리자 권한 실행 선택
- 프로필 시작 전 앱과 게임 종료 후 앱 자동 실행
- EXE 아이콘을 자동 썸네일로 사용하고 투명 아트워크에는 확대·블러 배경 합성
- EXE에 포함된 고해상도 아이콘을 우선 추출해 `data/cache/icons`에 투명 PNG로 캐시
- 썸네일 모드와 리스트 모드를 전환하고 선택한 모드를 저장
- 단일 정렬 메뉴에서 이름순·최신순과 오름차순·내림차순을 선택하고 상태 저장
- `게임`과 `도구 · 서버` 탭을 분리해 Asphyxia 같은 가상 서버와 보조 유틸리티 등록
- 도구·서버의 EXE/BAT/CMD 실행 및 현재 관리자 세션의 실행 상태 표시
...
게임·도구 편집 화면의 `고급 실행 옵션`은 기본적으로 접혀 있습니다. 펼치면 추가 인자, `관리자 권한으로 실행 (Windows UAC)`, 시작 전 앱과 종료 후 앱을 설정할 수 있습니다. 시작 전 앱은 게임보다 먼저 실행되지만 완료를 기다리지는 않으며, 종료 후 앱은 게임 프로세스가 실제로 끝난 뒤 실행됩니다. 보조 앱 경로는 프로필 폴더 기준 상대경로로 저장되고 EXE/BAT/CMD를 지원합니다. Configurator를 여는 경우에는 보조 앱을 실행하지 않습니다. BAT가 `start` 명령으로 게임을 분리 실행하면 BAT 종료 시점을 프로필 종료로 판단하므로, 종료 후 앱을 사용할 프로필에서는 게임 프로세스를 기다리는 BAT를 사용해야 합니다.

`+ 도구/서버`에서는 portable root 밖의 `../_tools` 같은 폴더도 상대경로로 등록할 수 있습니다. `가상 서버`와 `보조 도구` 중 유형을 선택하고 실행 파일과 작업 폴더를 지정합니다. 항목은 리스트나 썸네일에서 더블클릭해 실행하며, 관리자가 실행한 프로세스는 `도구 · 서버` 탭에서 실행 중으로 표시합니다. 관리자를 재시작한 뒤에도 이미 실행 중인 외부 프로세스를 다시 찾는 기능은 아직 포함하지 않습니다.
